...
abstract class BaseService
{
    ...
    /**
     * Apply date range on query.
     */
    protected function applyDateRange($query, $dateRange, $dateRangeStart = null, $dateRangeEnd = null)
    {
        if ($dateRangeStart && $dateRangeEnd) {

            try {

                $start =Carbon::parse($dateRangeStart)->startOfDay();
                $end   = Carbon::parse($dateRangeEnd)->endOfDay();
            ...
            switch ($dateRange) {
                ...
                case 'this_week':
                    ...
                    $query->whereBetween('created_at', [$start->toDateString(), now()->endOfDay()->toDateString()]);
                    break;
     ...
     * price:gt:50.00
     * price:lt:100.00
     * created_at:date_gte:1704067200
     * created_at:date_lte:1733011200
     *
     * -------------------------------------
     ...
     * -------------------------------------
     *
     * name:eq:John|price:gt:50.00|created_at:date_gte:1704067200|metadata->sms_credits:gte:100
     *
     * @return self
     */
    ...
    public function applyFilterOnQuery(string $filter): void
    {
        ...
        $operatorAlias = array_shift($extracted);
        ...
        if($isJsonField) {
            ...
            if($operator == 'in') {
                ...
                if(Str::startsWith($operatorAlias, 'date_')) {
                    $input1 = is_numeric($input1) && (int)$input1 >= 0 ? Carbon::createFromTimestamp((int) $input1)->toDateTimeString() : $input1;
                    $query = $this->getQuery()->where($column, $operator, $input1);
                }else{
                    $query = $this->getQuery()->where($column, $operator, $input1);
                }
    ...
    /**
     * Checks if valid Unix timestamp.
     *
     * @param mixed $value
     * @return boolean
     */
    function isValidUnixTimestamp(mixed $value): bool
    {
        if (! is_numeric($value)) {
            return false;
        }
        ...
        try {
            ...
            // Attempt to create a Carbon instance from the timestamp
            $date = Carbon::createFromTimestamp($ts);

            // Verify it round-trips correctly (handles overflow / invalid cases)
            return $date->timestamp === $ts;
    ...
    public static function extractColumnOperatorAndValue(string $input): array
    {
        ...
        $operatorAlias = array_shift($parts);
        $operator = self::convertOperatorToSymbol($operatorAlias);
    ...
    /**
     * Convert operator alias to operator.
     *
     * Input: gte or with prefix "date_" e.g "date_gte"
     * Output: >=
     *
     * @param string $operatorAlias
     * @return string|null
     */
    public static function convertOperatorToSymbol(string $operatorAlias): string|null
    {
        $operatorAlias = Str::replace('date_', '', $operatorAlias);
        ...
        return $operatorMap[$operatorAlias] ?? null;
    }
